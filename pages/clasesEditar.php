<?php
    ...
    if (isset($_GET['editarId'])) {
        ...
        $sql = "SELECT nombre_clase FROM clases where id_clase=$id";
        $result = $conn->query($sql); 
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $_SESSION["edit_nombreClase"] = $row['nombre_clase'];
            }
        }
        $conn->close();   
    }    
...
<div class=" bg-gray-200 h-screen w-screen">
                <h1 class=" p-3 text-2xl ml-2">Editar clase</h1>
                <div class="p-3 bg-white ml-5 w-[80%]">
                    <p>Aqui puede editar informacion de la clase</p>
                </div>
                <form class=" bg-white ml-5 w-[80%] p-5" action="../scripts/updateClase.php" method="post">
                    <div class="mt-2">
                        <span>Nombre de la materia</span>
                        <div class="w-[415] h-[52px] border-[1px] border-[#828282]">
                            <input class="ml-2 p-3 outline-none" type="text" name="nombreClase" id="nombreClase" value="<?= $_SESSION["edit_nombreClase"]?>" placeholder="Ingresa nombre">
                        </div>
                    </div>                       
                    <div class="">
